...
class ResultFormatter(BasePostProcessor):
    ...
    def _clean_content(self, content: str) -> str:
        """Clean OCR output content (legacy per-page cleaning only).

        NOTE: Full normalization (Unicode, bullets, etc.) is applied once
        by DocumentAssembler after cross-page stitching (#3, #17).
        This method only does minimal, safe cleanup for per-page output.
        """
        if content is None:
            return ""
