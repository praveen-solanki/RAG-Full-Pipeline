...
def load_pageindex_contexts(path: Path) -> List[tuple[PageIndexRow, Context]]:
    """
    Parse the pageindex JSON file and normalize every row to a Context.

    The file is a JSON object with a top-level "results" array. Each entry
    has a "question" field (aliased to query) and an "evidence_snippets" list
    which are joined into a single ContextChunk for the generator.
    Rows with status="error" are skipped (no evidence available).
    """
    ...
    with output_file.open(mode, encoding="utf-8") as fh:
        ...
        for row, context in bg_items:
            ...
            try:
                ...
                print(
                    f"[{done}/{total}] {row.retriever} ERROR: {exc!r} — query: "
                    f"{row.query[:80]!r}",
                    file=sys.stderr,
                )
                continue
            ...
            _write_result(fh, result)
            print(
                f"[{done}/{total}] {row.retriever:<12} OK  "
                f"({latency_ms:7.1f} ms)  {row.query[:60]!r}",
                file=sys.stderr,
            )
