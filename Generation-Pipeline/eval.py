...
#     #   vllm serve prometheus-eval/prometheus-7b-v2.0 --port 8011 \
...
#         --judge-base-url http://localhost:8011/v1 \
#         --run-tier1 --run-tier2
# """
...
#             model_name="/home/mpq3kor/.cache/huggingface/hub/models--BAAI--bge-m3/snapshots/5617a9f61b028005a4858fdac845db406aefb181",
...
#         )
#         if "pageindex" not in pivot.columns:
#             continue
#         # detect the non-pageindex retriever dynamically (bge_m3, hybrid_qdrr, etc.)
#         other_retrievers = [c for c in pivot.columns if c != "pageindex"]
#         if not other_retrievers:
#             continue
#         other = other_retrievers[0]

#         pairs = pivot.dropna(subset=["pageindex", other])
#         if pairs.empty:
#             continue

#         deltas = (pairs[other] - pairs["pageindex"]).to_numpy()
...
#                 f"{other}_mean": round(float(pairs[other].mean()), 4),
#                 f"mean_delta({other} - pageindex)": round(mean_delta, 4),
...
#     p.add_argument(
#         "--judge-base-url",
#         default="http://10.47.39.43:8011/v1",
...
# ---- BERTScore / DeBERTa overflow fix ------------------------------------ #
#
# bert_score.utils.sent_encode() does:
#
#     tokenizer.encode(sent, add_special_tokens=True,
#                      max_length=tokenizer.model_max_length, ...)
#
# DeBERTa's config sets model_max_length = int(1e30) as a sentinel for
# "no hard limit".  The HuggingFace *fast* tokenizer (Rust backend) receives
# that value in enable_truncation() and tries to cast it to usize / i64,
# which overflows and raises:  OverflowError: int too big to convert
#
# Patching the tokenizer object after the fact does NOT help because
# sent_encode reads tokenizer.model_max_length on every single call.
# The only reliable fix is to replace sent_encode itself with a version
# that caps max_length at a safe value (512 = DeBERTa's real window).
#
# We do this once at import time so it's in effect for every compute() call.

def _install_bertscore_overflow_fix(max_length: int = 512) -> None:
    """Monkey-patch bert_score.utils.sent_encode to cap max_length.

    The patch is idempotent: if it has already been applied (detected via
    a sentinel attribute) it does nothing.
    """
    import bert_score.utils as _bsu

    if getattr(_bsu.sent_encode, "_overflow_fix_applied", False):
    ...
    def _safe_sent_encode(tokenizer, sent):
        safe_max = min(
            max_length,
            getattr(tokenizer, "model_max_length", max_length) or max_length,
        )
        # model_max_length can be a huge float sentinel (e.g. 1e30); clamp it.
        if safe_max > max_length:
            safe_max = max_length
    ...
    _safe_sent_encode._overflow_fix_applied = True  # type: ignore[attr-defined]
    _bsu.sent_encode = _safe_sent_encode

    # bert_score.scorer imports sent_encode by name at module load time,
    # so we must patch that reference too.
    import bert_score.scorer as _bss
    if hasattr(_bss, "sent_encode"):
        _bss.sent_encode = _safe_sent_encode


_install_bertscore_overflow_fix()
# --------------------------------------------------------------------------- #
