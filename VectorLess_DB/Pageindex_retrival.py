...
# def parse_page_reference(page_reference: str) -> set:
#     """
#     Parse a page_reference string from the GT into a set of individual page numbers.
#     Handles all formats:
#       "Page 5"        -> {5}
#       "Pages 5-6"     -> {5, 6}
#       "Pages 3, 7"    -> {3, 7}         (non-contiguous)
#       "Pages 3, 7-9"  -> {3, 7, 8, 9}  (mixed)
#       "5-6"           -> {5, 6}
#     Returns empty set if parsing fails — caller checks for empty before using.
#     """
#     if not page_reference:
#         return set()

#     # Strip leading "Page"/"Pages" prefix (case-insensitive)
#     cleaned = re.sub(r"(?i)^pages?\s*", "", page_reference.strip())

#     pages = set()
#     # Split on commas to handle non-contiguous refs like "3, 7-9"
#     for part in cleaned.split(","):
#         part = part.strip()
#         # Range like "5-6" or "5-6"
...
# def check_retrieval_overlap(pages_used: str, gold_pages: set) -> dict:
#     """
#     Step 5a — No LLM needed.
#     Compares the pages actually extracted by the pipeline (pages_used) against
#     the gold page set parsed from page_reference in the GT.

#     pages_used  : compact string already produced by get_page_range_string,
#                   e.g. "3-6,12"  — these are real PDF page numbers.
#     gold_pages  : set of int page numbers from parse_page_reference, e.g. {5, 6}

#     Only recall is meaningful here:
#       recall = |overlap| / |gold_pages|
#     Precision is NOT computed — nodes are large sections that always span more
#     pages than the narrow gold reference, making precision structurally misleading.

#     Returns:
#       gold_pages      : sorted list of gold page numbers
#       retrieved_pages : sorted list of pages actually extracted
#       overlap_pages   : sorted list of pages in both
#       page_recall     : overlap / gold  (None if gold_pages is empty)
#     """
#     # Parse pages_used string into a set of ints
#     # Format: "3-6,12,15-17"
#     retrieved_pages = set()
#     for part in pages_used.split(","):
#         part = part.strip()
#         m = re.match(r"^(\d+)-(\d+)$", part)
#         if m:
#             retrieved_pages.update(range(int(m.group(1)), int(m.group(2)) + 1))
#         elif part.isdigit():
#             retrieved_pages.add(int(part))

#     overlap = gold_pages & retrieved_pages
#     page_recall = round(len(overlap) / len(gold_pages), 4) if gold_pages else None
...
#     # Build one normalised string from all retrieved page content — done ONCE
#     full_text     = " ".join(
...
#         # 5a-i  Page recall — gold pages (from page_reference) vs pages actually extracted
#         gold_pages = parse_page_reference(page_reference)
#         if gold_pages:
#             retrieval_eval = check_retrieval_overlap(page_range, gold_pages)
#         else:
#             retrieval_eval = {
#                 "gold_pages":      [],
#                 "retrieved_pages": [],
...
#                 "note": f"could not parse page numbers from: {page_reference!r}",
#             }
#         print(f"  → Page recall: {retrieval_eval.get('page_recall', 'N/A')} "
#               f"(gold={retrieval_eval.get('gold_pages')} "
#               f"retrieved={retrieval_eval.get('retrieved_pages')})")
...
#             "source_document":  pdf_name,
#             "page_reference":   page_reference,
#             "gold_pages":       sorted(parse_page_reference(page_reference)),
#             "answer":           "",
#             "ground_truth":     ground_truth,
#             "status":           "error",
#             "error":            str(e),
...
#     Metrics computed:
#       Page recall     — fraction of gold pages that were actually retrieved.
#                         Only for questions where page_reference was parseable.
#                         Precision and F1 are NOT computed — nodes are large sections
#                         that always span more pages than the narrow gold reference,
#                         making precision structurally misleading.

#       Evidence recall — fraction of gold evidence snippets found (substring) in
#                         retrieved text. Only for questions that have snippets.
#                         This is the most reliable retrieval signal.

#       Answer quality  — LLM judge: verdict, correctness score, completeness score,
#                         hallucination. Denominator for accuracy is total questions
#                         (errors count as wrong — no inflation from excluding failures).
#     """
#     total      = len(results)
...
#     # ── Page recall ───────────────────────────────────────────────────────────
#     # Only include questions where gold_pages was parseable (page_recall is a float)
#     ret_evaluable = [
#         r for r in successful
#         if isinstance(r.get("retrieval_eval", {}).get("page_recall"), float)
#     ]
#     ret_excluded  = n_success - len(ret_evaluable)
#     avg_page_recall = (
#         round(sum(r["retrieval_eval"]["page_recall"] for r in ret_evaluable) /
#               max(len(ret_evaluable), 1), 4)
#         if ret_evaluable else None
#     )

#     # ── Evidence-snippet recall ───────────────────────────────────────────────
#     # Only average over questions that have evidence_snippets
#     ev_evaluable = [
#         r for r in successful
...
#     avg_evidence_recall = (
#         round(sum(r["evidence_recall_eval"]["evidence_recall"] for r in ev_evaluable) /
#               max(len(ev_evaluable), 1), 4)
...
#     # accuracy denominator = total questions (errors count as wrong)
#     accuracy = round(correct / max(total, 1), 4)

#     judge_evaluable  = [r for r in successful
#                         if r.get("evaluation", {}).get("correctness_score") is not None]
#     avg_correctness  = (
#         round(sum(r["evaluation"]["correctness_score"]  for r in judge_evaluable) /
#               max(len(judge_evaluable), 1), 4)
#         if judge_evaluable else None
#     )
#     avg_completeness = (
#         round(sum(r["evaluation"]["completeness_score"] for r in judge_evaluable) /
#               max(len(judge_evaluable), 1), 4)
#         if judge_evaluable else None
#     )
...
#                     "page_recall_evaluable": 0, "evidence_evaluable": 0,
#                     "_page_recall_sum": 0.0, "_ev_recall_sum": 0.0,
#                     "_correctness_sum": 0.0,
#                 }
#             g = groups[key]
#             g["total"] += 1
...
#             if isinstance(ret.get("page_recall"), float):
#                 g["page_recall_evaluable"] += 1
#                 g["_page_recall_sum"] += ret["page_recall"]
#             # evidence recall
...
#             g["avg_page_recall"]     = (
#                 round(g["_page_recall_sum"] / max(g["page_recall_evaluable"], 1), 4)
#                 if g["page_recall_evaluable"] else None
#             )
#             g["avg_evidence_recall"] = (
#                 round(g["_ev_recall_sum"] / max(g["evidence_evaluable"], 1), 4)
#                 if g["evidence_evaluable"] else None
#             )
#             g["avg_correctness"]     = round(g["_correctness_sum"] / max(g["total"], 1), 4)
#             del g["_page_recall_sum"], g["_ev_recall_sum"], g["_correctness_sum"]
#         return groups
...
#         "summary": {
#             # ── Page recall ─────────────────────────────────────────────────
#             "page_recall_evaluable":         len(ret_evaluable),
#             "page_recall_excluded":          ret_excluded,
...
#     print(f"")
#     print(f"   [Retrieval — page recall]")
#     print(f"   Evaluable (parseable page_ref) : {s['page_recall_evaluable']}  "
#           f"(excluded: {s['page_recall_excluded']})")
#     print(f"   Avg page recall                : {s['avg_page_recall']}")
#     print(f"   Note: recall = gold pages retrieved / total gold pages.")
#     print(f"         Precision not computed — nodes span more pages than")
#     print(f"         narrow gold references, making it structurally misleading.")
#     print(f"")
...
def build_documents(pdf_name: str, structure: dict) -> tuple[dict, str]:
    ...
    documents = {
        doc_id: {
            ...
            "doc_description": structure.get("doc_description", ""),
            "path":            pdf_path,
            "structure":       structure.get("structure") or structure.get("hierarchy", []),
            "pages":           cached_pages,   # None → retrieve.py opens PDF normally
        }
    }
    return documents, doc_id
...
def build_doc_registry(tree_dir: str) -> list[dict]:
    ...
    for fname in sorted(os.listdir(tree_dir)):
        ...
        try:
            ...
            if not doc_desc:
                # Fallback: generate a description from root node titles
                nodes    = data.get("structure") or data.get("hierarchy", [])
                titles   = [n.get("title", "") for n in nodes[:5] if n.get("title")]
                doc_desc = f"Document covering: {', '.join(titles)}" if titles else doc_id
            registry.append({
                "doc_id":          doc_id,
