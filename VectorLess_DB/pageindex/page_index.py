...
def toc_detector_single_page(content, model=None):
    ...
    if not json_content or 'toc_detected' not in json_content:
        logging.warning(
            f"toc_detector_single_page: LLM returned no usable response "
            f"(model={model}, response={response!r:.200}). Defaulting to 'no'."
        )
        return 'no'
    return json_content['toc_detected']
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    # Issue C: extract_json returns {} on parse failure; default to 'no' to avoid KeyError
...
def detect_page_index(toc_content, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    # Issue D: mirror the guard already present in toc_detector_single_page
    if not json_content or 'page_index_given_in_toc' not in json_content:
        logging.warning(
            f"detect_page_index: LLM returned no usable response "
            f"(model={model}, response={response!r:.200}). Defaulting to 'no'."
        )
        return 'no'
    return json_content['page_index_given_in_toc']
...
def _extract_toc_list(parsed):
    """
    Problem 4: tolerate several JSON shapes the model may return instead of
    the canonical {"table_of_contents": [...]}.
    Accepted shapes (in priority order):
      - a bare list                        -> use directly
      - {"table_of_contents": [...]}       -> canonical
      - {"toc": [...]}, {"contents": [...]}, {"sections": [...]} -> common variants
      - a dict with exactly one key whose value is a list -> use that list
    Returns the list, or raises ValueError if nothing matches.
    """
    if isinstance(parsed, list):
        return parsed
    if isinstance(parsed, dict):
        for key in ('table_of_contents', 'toc', 'contents', 'sections'):
            if key in parsed and isinstance(parsed[key], list):
                return parsed[key]
        # Last resort: single-key dict whose value is a list
        if len(parsed) == 1:
            only_value = next(iter(parsed.values()))
            if isinstance(only_value, list):
                return only_value
    raise ValueError(f"Cannot locate TOC list in parsed structure: {type(parsed)}")
...
def toc_transformer(toc_content, model=None):
    ...
    if if_complete == "yes" and finish_reason == "finished":
        ...
        cleaned_response = convert_page_to_int(_extract_toc_list(parsed))
        return cleaned_response
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        # Issue G: slice to position+1 (keep the '}'), not position+2 (which appends a stray char)
        position = last_complete.rfind('}')
        if position != -1:
        ...
        # Problem 3: always append the continuation regardless of whether it starts with ```json.
        # Strip markdown fences if present, then unconditionally append.
        if new_complete.startswith('```json') or new_complete.startswith('```'):
            new_complete = get_json_content(new_complete)
        last_complete = last_complete + new_complete
    ...
    parsed = extract_json(last_complete)
    # Problem 4: tolerate various JSON shapes returned by the model
    cleaned_response = convert_page_to_int(_extract_toc_list(parsed))
    return cleaned_response
...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    ...
    while i < len(page_list):
        ...
        try:
            ...
            if logger:
                logger.warning(f"toc_detector_single_page failed for page {i}: {e}. Skipping.")
            detected_result = 'no'
        if detected_result == 'yes':
            if logger:
                logger.info(f'Page {i} has toc')
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    # Problem 2: instead of crashing, use the same continuation loop pattern as extract_toc_content.
    chat_history = [
        {"role": "user", "content": prompt},
        {"role": "assistant", "content": response},
    ]
    continuation_prompt = "Please continue the generation of the JSON structure, directly output the remaining part."
    ...
    while finish_reason != 'finished':
        attempt += 1
        if attempt >= max_attempts:
            logging.warning('generate_toc_continue: hit max continuation attempts, returning best effort')
            break
        chat_history = [
            {"role": "user", "content": prompt},
            {"role": "assistant", "content": response},
...
def generate_toc_init(part, model=None):
    ...
    while finish_reason != 'finished':
        attempt += 1
        if attempt >= max_attempts:
            logging.warning('generate_toc_init: hit max continuation attempts, returning best effort')
            break
        chat_history = [
            {"role": "user", "content": prompt},
            {"role": "assistant", "content": response},
...
def process_no_toc(page_list, start_index=1, model=None, max_token_num_each_node=20000, logger=None):
    page_contents=[]
    token_lengths=[]
    for page_index in range(start_index, start_index+len(page_list)):
        page_text = f"<physical_index_{page_index}>\n{page_list[page_index-start_index][0]}\n<physical_index_{page_index}>\n\n"
        page_contents.append(page_text)
        token_lengths.append(count_tokens(page_text, model))
    # Issue A: pass max_token_num_each_node so the config value is actually respected.
    # Without this the hardcoded default of 20000 is always used regardless of config.
    group_texts = page_list_to_group_text(page_contents, token_lengths, max_tokens=max_token_num_each_node)
    logger.info(f'len(group_texts): {len(group_texts)}')
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, max_token_num_each_node=20000, logger=None):
    page_contents=[]
    token_lengths=[]
    toc_content = toc_transformer(toc_content, model)
    logger.info(f'toc_transformer: {toc_content}')
    ...
    # Issue A: pass max_token_num_each_node so config value is respected (not hardcoded 20000)
    group_texts = page_list_to_group_text(page_contents, token_lengths, max_tokens=max_token_num_each_node)
    logger.info(f'len(group_texts): {len(group_texts)}')
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    ...
    # Issue B: calculate_page_offset returns None when no matching pairs are found.
    # add_page_offset_to_toc_json would then crash with TypeError (int + None).
    # Fall back gracefully: return the TOC as-is so the caller can try the next mode.
    if offset is None:
        logging.warning('process_toc_with_page_numbers: could not calculate page offset (no matching pairs). Returning TOC without physical indices.')
        return toc_with_page_number
...
async def single_toc_item_index_fixer(section_title, content, model=None):
    ...
    response = await llm_acompletion(model=model, prompt=prompt)
    json_content = extract_json(response)
    # Issue E: extract_json returns {} on LLM failure; raise so asyncio.gather captures it
    # as an Exception (return_exceptions=True) rather than crashing the whole pipeline.
    if not json_content or 'physical_index' not in json_content:
        raise KeyError(
            f"single_toc_item_index_fixer: 'physical_index' missing from LLM response "
            f"for section '{section_title}'. Response: {response!r:.200}"
        )
    return convert_physical_index_to_int(json_content['physical_index'])
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Early return if we don't have valid physical indices
    if last_physical_index is None or last_physical_index < len(page_list)/2:
        # Problem 6: return -1 (not 0) to distinguish "output truncated / incomplete coverage"
        # from "verified and found bad". meta_processor treats -1 as a truncation signal and
        # cascades to the next mode rather than mistaking it for a verification failure.
        return -1, []
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if mode == 'process_toc_with_page_numbers':
        ...
        toc_with_page_number = process_toc_no_page_numbers(toc_content, toc_page_list, page_list, model=opt.model, max_token_num_each_node=opt.max_token_num_each_node, logger=logger)
    else:
        toc_with_page_number = process_no_toc(page_list, start_index=start_index, model=opt.model, max_token_num_each_node=opt.max_token_num_each_node, logger=logger)
    ...
    # Problem 6: accuracy == -1 is a sentinel meaning the output was truncated (coverage < 50%
    # of the document), NOT that verification found it bad. Retry the same mode rather than
    # cascading to the next mode, which would give up on a recoverable situation.
    if accuracy == -1:
        logging.warning(f"meta_processor: verify_toc returned sentinel -1 (truncated output) for mode={mode}. Retrying same mode.")
        if mode == 'process_toc_with_page_numbers':
            return await meta_processor(page_list, mode='process_toc_no_page_numbers', toc_content=toc_content, toc_page_list=toc_page_list, start_index=start_index, opt=opt, logger=logger)
        elif mode == 'process_toc_no_page_numbers':
            return await meta_processor(page_list, mode='process_no_toc', start_index=start_index, opt=opt, logger=logger)
        else:
            raise Exception('Processing failed: output was truncated in all modes')
