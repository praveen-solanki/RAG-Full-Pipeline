"""
PageIndex RAG Pipeline v3
=========================
Upgraded from v2 with PageIndex-aligned retrieval:

Changes from v2:
  1. Fixed structure key bug: structure.get("structure", []) instead of .get("nodes", structure)
  2. tree_search now returns only node_id list — no page numbers from LLM (PageIndex style)
  3. resolve_nodes() looks up page ranges programmatically from the tree (no LLM hallucination)
  4. add_prefix_summaries() propagates ancestor context to every node before tree search
  5. get_page_range_string() deduplicates pages to reduce over-retrieval and improve precision
...
STRUCTURE_DIR   = "/home/olj3kor/praveen/Nemo_NVIDIA/pymupdf_data"
PDF_DIR         = "/home/olj3kor/praveen/Image_dataset_generation/pdfs"
OUTPUT_FILE     = "/home/olj3kor/praveen/rag_pymupdf_results.json"
...
def build_documents(pdf_name: str, structure: dict) -> tuple[dict, str]:
    ...
    documents = {
        doc_id: {
            ...
            "path":            pdf_path,
            "structure":       structure.get("structure", []),  # fixed: our JSON uses "structure" not "nodes"
...
def tree_search(query: str, tree_structure_json: str) -> list[str]:
    """
    ...
    Prompt matches the official PageIndex prompt exactly.
    """
...
def process_question(q: dict, index: int, total: int) -> dict:
    ...
    try:
        ...
        # Step 2 — Tree search: LLM returns only node_ids (no page numbers)
        tree_json   = get_document_structure(documents, doc_id)
        node_ids    = call_with_retry(tree_search, query, tree_json)
